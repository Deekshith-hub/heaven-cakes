...
const SkeletonCard = () => (
  <div className="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-gray-100 dark:border-slate-700 h-full flex flex-col">
    <div className="h-48 bg-gray-100 dark:bg-slate-700 rounded-xl animate-pulse mb-4"></div>
    <div className="h-4 bg-gray-100 dark:bg-slate-700 rounded w-3/4 mb-2 animate-pulse"></div>
    <div className="h-3 bg-gray-100 dark:bg-slate-700 rounded w-full mb-4 animate-pulse"></div>
    <div className="mt-auto flex justify-between pt-3 border-t border-gray-50 dark:border-slate-700">
      <div className="h-5 bg-gray-100 dark:bg-slate-700 rounded w-1/3 animate-pulse"></div>
      <div className="h-8 w-8 bg-gray-100 dark:bg-slate-700 rounded-full animate-pulse"></div>
    </div>
  </div>
);
...
export default function Home({ addToCart, searchTerm }: HomeProps) {
  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-hidden w-full max-w-[100vw]">
      ...
      {/* Hero Section */}
      <div className="relative bg-[#F8FAE5] dark:bg-slate-800 pt-24 pb-16 px-6 text-center mb-10 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto animate-slide-up">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-base md:text-xl max-w-lg mx-auto leading-relaxed mb-8">
      ...
      <div id="shop-section" className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <h2 className="text-2xl md:text-3xl font-bold text-[#1A202C] dark:text-gray-100">Our Collection</h2>
            <div className="flex gap-2 p-1 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-x-auto max-w-full w-full md:w-auto scrollbar-hide">
                {['All', 'Ice Cake', 'Bento', 'Cupcake'].map(cat => (
                <button key={cat} onClick={() => setFilter(cat)} className={`flex-1 md:flex-none px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-bold transition-all whitespace-nowrap ${filter === cat ? 'bg-[#43766C] text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>{cat}</button>
                ))}
            </div>
        </div>
